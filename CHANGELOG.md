...
- `--help` prints a chuckfmt-specific blurb followed by clang-format's options
- `--version` prints chuckfmt's version followed by clang-format's version
- Both flags no longer block waiting on stdin

## [0.2.1] - 2026-04-14

### Fixed

- `--assume-filename` now uses the real file's directory so clang-format finds `.clang-format` config relative to input files (previously always searched from CWD)
...
[0.2.2]: https://github.com/aik2mlj/chuckfmt/compare/v0.2.1...v0.2.2
[0.2.1]: https://github.com/aik2mlj/chuckfmt/compare/v0.2.0...v0.2.1
